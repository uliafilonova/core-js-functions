...
function memoize(func) {
  const cache = {};
  return function my() {
    if (Object.prototype.hasOwnProperty.call(cache, 'f')) {
      return cache.f;
    }
    cache.f = func();
    return cache.f;
...
function logger(func, logFunc) {
  return function my(...args) {
    ...
    logFunc(`${func.name}(${formatt}) starts`);

    const result = func(...args);

    logFunc(`${func.name}(${formatt}) ends`);
